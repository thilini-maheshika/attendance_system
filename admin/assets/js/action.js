...
function loadclass(ele, id, field, table, tbl_id, sec_id) {
    var value = document.getElementById(ele.id).value;
    if (table == 'teacher') {
        isSectionAssigned(sec_id)
            .then(function(isAssigned) {
                if (!isAssigned) {
                    EditData(ele, id, field, table, tbl_id);
                    EditDatawithValue("1", value, "is_assigned", "section", "sec_id");
                } else {
                    sweetAlert2(warning, 'Section is already assigned to a teacher!');
                }
            })
...
function isSectionAssigned(sec_id) {
    ...
    .then(function(data) {
        return data === 'true'; // return a boolean value instead of a string
    })
    .catch(function(error) {
        console.log(`Error ${error}`);
        sweetAlert2(warning, 'Something went wrong. Please try again!');
